...
```php
// BaseModel <nome progetto>corrente - IMPLEMENTA GIÀ HasMedia + InteractsWithMedia
...
```php
// Modules/<nome progetto>p/Filament/Resources/PatientResource.php - getFormSchema()
...
'health_card' => \Modules\UI\Filament\Components\SpatieDocumentUpload::forHealthCard()
    ->label(trans('<nome progetto>atients.fields.health_card.label'))
    ->helperText(trans('<nome progetto>atients.fields.health_card.help')),
```
...
```php
// Modules/<nome progetto>p/Models/User.php - Aggiunta registerMediaCollections
...
```php
// Modules/<nome progetto>p/Models/User.php - Accessors di transizione
...
<div class="grid grid-cols-2 gap-4">
    @if($patient->hasMedia('tessere_sanitarie'))
        <div class="document-preview">
            <h4>{{ __('<nome progetto>atients.health_card') }}</h4>
            ...
            <a href="{{ $patient->getFirstMediaUrl('tessere_sanitarie') }}"
               ...
               class="text-blue-600 text-sm">
                {{ __('<nome progetto>ommon.view_document') }}
            </a>
        </div>
    @endif
    ...
    @if($patient->hasMedia('certificazioni_isee'))
        <div class="document-preview">
            <h4>{{ __('<nome progetto>atients.isee_certificate') }}</h4>
            ...
            <a href="{{ $patient->getFirstMediaUrl('certificazioni_isee') }}"
               target="_blank"
               class="text-blue-600 text-sm">
                {{ __('<nome progetto>ommon.download_pdf') }}
            </a>
        </div>
    @endif
</div>
...
- [<nome progetto>corrente Models Architect<nome progetto>rogetto corrente/docs/models-architecture.md)
...
*Versione: 1.0*
*Compatibilità: Laraxot <nome progetto>corrente, Spatie Media Library 11.x, Filament 3.x*
